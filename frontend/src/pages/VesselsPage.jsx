...
export default function VesselsPage() {
    ...
    const loadVessels = async (f = filters) => {
        setLoading(true);
        try {
            const data = await fetchVessels(f);
            setVessels(data);
        } catch (err) {
    ...
    const handleReset = () => {
        const cleared = { name: "", type: "", flag: "", cargo_type: "" };
        setFilters(cleared);
        loadVessels(cleared);
    };
        ...
        <div className="vessels-page">
            ...
            {loading ? (
                ...
                <div className="vessels-empty">
                    <p>No vessels found. Try different filters or add vessels via the admin panel.</p>
                </div>
            ) : (
                <div className="vessels-table-wrap">
                        ...
                        <tbody>
                            {vessels.map(v => (
                                <tr key={v.id}>
                                    ...
                                    <td className="mono">{v.imo_number}</td>
                                    ...
                                    <td>
                                        <Link to={`/vessels/${v.id}`} className="button button--sm">
                                            View
                                        </Link>
                                    </td>
                                </tr>
                    ...
                    <p className="vessels-count">{vessels.length} vessel{vessels.length !== 1 ? "s" : ""}</p>
                </div>
            )}
        </div>
    );
