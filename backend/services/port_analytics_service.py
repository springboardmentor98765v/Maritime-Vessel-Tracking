...
def get_port_analytics(port):
    ...
    # ── PDF Step 5 — trigger congestion notification ──
    try:
        from services.congestion_notification_service import (
            send_congestion_notification
        )
        send_congestion_notification(port.name, congestion_score)
    except Exception as e:
        logger.error(f"Error sending congestion notification: {e}")
